...
* Version 1.1   : This version of this program contains a Bubble Sort and Linear         *
*               : Search algorithms                                                      *
******************************************************************************************/
...
/*****************************************************************************************
*                 Bubble Sort - Sorts courses in alphabetical order                   *
******************************************************************************************/
void bubbleSort(vector<vector<string>>& n_courses) {
    //define  variables for loops and min index
    int i, j;

    string courseIdOne, temp;

    // outter for loop to determine min index
    for (i = 1; i < n_courses.size(); ++i) { 							//O(N)

        // inner loop to compare indexes 
        for (j = 0; j < n_courses.size() - 1; ++j) {					// O(N)

            //
            courseIdOne = n_courses[j][0];								// O(1)

            //
            if (courseIdOne.compare(n_courses[i][0]) > 0) {				// O(1 + 3)
                // swap the current minimum with smaller one found
                temp = n_courses[i][0];
                n_courses[i][0] = courseIdOne;
                n_courses[j][0] = temp;
            }
        }
    }
