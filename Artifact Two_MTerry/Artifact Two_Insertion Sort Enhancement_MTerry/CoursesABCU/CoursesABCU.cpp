...
* Version 1.2   : This version of this program contains a Insertion Sort and Linear      *
*               : Search algorithms                                                      *
******************************************************************************************/
...
/*****************************************************************************************
*                 Insertion Sort - Sorts courses in alphabetical order                   *
******************************************************************************************/
void insertionSort(vector<vector<string>>& n_courses) {
    //define  variables for loops and min index
    int i, j, key;

    string courseIdOne, courseIdTwo, temp;

    for (i = 1; i < n_courses.size() - 1; ++i) {		// O(N)        
        ...
        //
        courseIdOne = n_courses[j][0];				// O(1)
        courseIdTwo = n_courses[key][0];			// O(1)

        while (j >= 0 && (courseIdOne.compare(courseIdTwo) > 0)) {		// O(N)

            temp = courseIdOne;					// O(1)
            courseIdOne = courseIdTwo;			// O(1)
            courseIdTwo = temp;					// O(1)

            // insert lower and upper course
            n_courses[j][0] = courseIdOne;		// O(1)
            n_courses[key][0] = courseIdTwo;	// O(1)
        }
    }

    // Big O Analysis Calculation
    // f(N) = O(N) + O(1) + O(1) + O(N) + O(1) + O(1) + O(1) + O(1) + O(1)
    // Big O = O(N^2)
}
